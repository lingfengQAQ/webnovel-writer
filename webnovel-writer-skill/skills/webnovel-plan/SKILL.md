...
export SKILL_ROOT="${WEB_NOVEL_LIB}/skills/webnovel-plan"
...
```bash
cat "${WEB_NOVEL_LIB}/templates/output/大纲-卷节拍表.md"
```
...
```bash
cat "${WEB_NOVEL_LIB}/templates/output/大纲-卷时间线.md"
```
...
```bash
cat "${WEB_NOVEL_LIB}/references/genre-profiles.md"
cat "${WEB_NOVEL_LIB}/references/shared/strand-weave-pattern.md"
```

按需加载：

```bash
cat "${WEB_NOVEL_LIB}/references/shared/cool-points-guide.md"
...
```bash
cat "${WEB_NOVEL_LIB}/references/reading-power-taxonomy.md"
