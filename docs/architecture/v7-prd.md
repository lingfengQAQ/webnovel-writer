...
② 建书        打开 AI 工具说"开始写书"→ 引导式问答生成 book.yaml、作品契约、总纲、第一卷卷纲
③ 写第一章    确认整份细纲（含章级知识选择与计划对象引用）→ AI 写稿 → 机检 → 两审 → 审稿 → 定稿
④ 日常        每次打开，AI 先报全书近况（写到哪、哪些伏笔悬了太久、连续几章钩子弱）
              → 继续写下一章，或说"改第 N 章""把这条感情线收了"
⑤ 卷末        卷复盘：伏笔清账、卷摘要、下卷卷纲、伏笔机会扫描；同时复盘作品契约，有问题只提修订候选
⑥ 发布        一键导出干净正文（单章/范围/全书，去 front matter），贴到起点/番茄
```

体验底线：全程看不到 git、看不到报错堆栈、看不到英文术语；书的核心内容只在 定稿/大纲/作品契约/文风/工作区 五个中文目录中。
...
├── 作品契约/
│   ├── 作品契约.md             # ★ 唯一书级契约真源
│   └── 知识选择记录.md         # 实际采用/变体与真实发生的拒绝、修改、自定义证据；不进日常上下文
├── 文风/
│   ├── 文风铁律.md             # ★ 原「风格宪法.md」；纯作者品味（禁词/口癖/来自否决的规则）
│   └── 金句库/战斗.md …
├── 工作区/                     # ── 没定稿的一切，默认不进 git ──
...
- **创作知识**：不是账本条目，按调用阶段分为十维——作品契约层（题材/流派/创意约束）、故事对象层（设定/人物/命名）、篇章执行层（节拍/场景/技法/追读）。每条只有一个 canonical 归属，跨维使用靠组合。建书只对真实未决项提供少量整体方向包，最终蒸馏为 `作品契约/作品契约.md`；故事对象先进入 `大纲/创作设计/`，正文首次出现并定稿后才转为事实；章级知识随整份细纲确认，备料与两审只读取最终选择的切片。固定路由只做名称归一和候选范围定位，不决定具体创意或剧情。
...
| # | 阶段 | 执行体 | 说明 |
|---|------|--------|------|
| 1 | 起草细纲 | 脚本读全书近况 + AI 拟 | 生成 `工作区/细纲.md`；声明本章定位，并从节拍/场景/技法/追读每维最多三条候选中选择、组合、变体或自定义；按 ID/名称显式声明本章用到的计划对象。候选是材料，不是固定答案 |
| 2 | 确认细纲 | **作者** | 整份确认本章任务、章级知识选择、真实变体与计划对象引用；不逐项打断。自动模式可跳过停顿，但定稿仍归作者 |
| 3 | 备料 | 脚本 | 组装 `工作区/本章写作材料.md`：全书近况+要写到的事+定稿/批内事实+信息差边界+近章结尾+反复读清单+文风锚点+作品契约所需小节+显式引用的计划对象+细纲最终选择的落笔切片；**默认用精准片段**，不回读当前通用书级条目 |
...
| 6 | 两审 | AI ×2（各自新鲜上下文） | **事实审查**核对设定、时间线、连贯、角色、逻辑、要写到的事、泄密候选、履历证据、计划对象与既有事实，并可输出 `factChanges`；冲突/歧义交作者裁决。**编辑审**核对结构、节奏、商业性、主角动机、作品契约与所选章级知识的审稿切片 |
| 7 | 审稿 | **作者** | `工作区/审稿.md` = 草稿 + 三句话审意见 + 待确认新人名 + 章摘要（扫一眼可改）。动作：接受 / 改完接受 / 打回。手动模式逐章审；自动模式按批次审（§2.3）。**定稿永远需要作者敲定，没有"自动定稿"** |
| 8 | 定稿 | 脚本，**原子** | 一次 git commit：正文与章级最终知识选择进定稿、无冲突/已裁决事实转正、对应计划对象删除、设定/时间线/名册更新、三类条目写入履历、章摘要入档；commit 成功后才清工作区。要么完成要么原样保留 |

写评分离：写稿与两审必须在不同上下文中进行；机检先于 AI 审。完整模式两审各用独立上下文，降级模式在单一审稿上下文中顺序执行并如实声明。
...
| 1 | 工作目录无书 / 当前书不存在 | 建书引导：名称路由只做 canonical 归一，作者确认作品契约、知识选择和大纲后一次落盘 |
| 2 | 作品契约/定稿/大纲/文风有未入账手改 | 提议补登（系统适应作者，不报错） |
| 3 | 工作区有未完成流程 | 从中断的阶段继续 |
| 4 | 最新定稿章声明收卷且尚未复盘 | 卷复盘，同时检查作品契约是否需要提出修订候选 |
| 5 | 章号到达体检周期 | 体检 |
| 6 | 其余 | 起草新章细纲 |
...
- 读作品契约在当前阶段需要的小节 / 按 ID 或名称读一个尚未转正的计划对象
- 按真实未决问题查询某一知识维度最多三条候选 / 只读细纲已选知识的落笔或审稿切片
...
| # | 问题 | 解法 | 版本 | 验收方式 |
|---|---|---|---|---|
| 1 | 客观事实吃书（修为/资产/物品） | 故事对象先在 `大纲/创作设计/` 规划，首次正文定稿时由事实审查给出差异；无冲突事实与章节原子转正，冲突须作者裁决 | 7.0 | 计划不被当事实；转正、冲突、hash 漂移和写入故障均有原子性用例 |
| 2 | 时间线混乱 | 每章强制时间锚点；时间线按卷拆分，append-only | 7.0 | 从第 1 章起强制；体检报告缺时间锚点的章为零 |
| 3 | 信息差泄密 | 登记表（每条一文件：知情人/读者已知/关键词）+ 机检出候选 + 事实审查判真伪；写作材料注入出场人物知识边界 | 7.0 | 构造泄密样章，两审能逮住；误报不拦截流程 |
...
- 骨架：状态机单入口、写章流程八阶段（机检/两审分工）、三类账本（声明制）、故事对象计划→事实生命周期、时间线（按卷）、分层摘要、改稿三档、git 隐身全套（含健康检查与"回到第 N 章"）、精准读取接口
- 高呼声卖点：自动模式（连写，**按批次定稿**）、作品契约长期边界、十维知识少量候选与自由组合、信息差登记（候选制泄密检查）
- 配套：npx 安装器（工作目录布局）+ update、`/migrate`、干净导出、AGENTS.md
- 知识与文风基础：十维通用知识（开发期策展、作者侧只读）+ 作品契约 + 文风铁律 + 禁词/句式机检 + 高频意象统计；金句库目录与手动放入
- 宿主：一级（Claude Code + Codex），亲测过 smoke

### v7.x（增量）

- 金句库自动收割（候选制：作者改写且字数相近才候选，净删减不算；**对话触发**：作者聊天中夸某段也触发；最终一律作者审核入库）
- **按需静态简报**（单文件 HTML，内联样式无外链无服务器；账本全貌、体检统计与情绪波形、时间线浏览——"全书近况"管对话里的决策瞬间，浏览级信息归简报。**生成方式：脚本从缓存渲染固定模板，零 token，AI 不参与 HTML 构造**；模板随插件分发并入模板哈希清单；可选"近况点评"小节由 AI 以数据形式提供给脚本嵌入）
...
- 旧 v7 书仓迁移、旧路径双读、旧 schema 兼容；作者侧知识投稿/评分/自动扩库；按固定路由替作者选择创意或剧情答案
...
| **beta** | 用 v7 真实写一本书到 50 章（自用验证，覆盖作品契约建书→计划对象→章级知识选择→两审→定稿→吃书→卷复盘）；Windows 中文路径全链路 CI 绿；"删光 .cache 全量重建" CI 绿 |
...
| 上下文包 | 本章写作材料 | |
| 合同 | 本章要写到的事 | |
| 镜头评审 | 两审（事实审查/编辑审） | 事实可验证性与编辑判断分开；读者体验由编辑审结合作品契约和所选追读知识核对 |
...
| 风险 | 控制 |
|---|---|
| 两审在弱模型宿主上质量参差 | 两份任务书单源；事实审查给核对清单与结构化事实差异；无 subagent 时用单上下文顺序审稿并如实声明 |
| 固定路由使不同作品或章节收敛到同一答案 | 路由只归一名称；候选每维最多三条且允许组合、变体、拒绝、自定义；近期历史只软降权，不自动选择 |
...
## 10. 历史 spec 修订指令（PRD 1.0 下行记录）

本节记录 1.0 对当时两份 spec 的修订要求，不再代表 1.6 当前实现；其中“三审”等旧口径已由后续决策替代。当前格式以 story repo spec 0.17 和 multi-agent spec 现行版为准。
...
| references（题材模板/追读力/爽点节奏及 v6 候选资产） | 按十维宪章逐条重写策展；具体模板不平移，无法抽象为可迁移原理者删除 |
...
- [x] 十维知识有清晰消费者与降级边界；固定路由不替作者选择创意，作品契约/计划对象/章级选择均有唯一持久化落点
