...
> 状态：1.4（2026-07-08 修订，恩怨清算档位三档扩五档：有仇必报/宿命必偿/恩怨分明/以直报怨/留有余地，依据任务 07-08-kb-ima-distill-integration 与 story-repo-spec 0.16 决策 43；1.3 于 2026-07-07 修订，知识层从「平移继承」改为「重写适配」：三层声明路由（建书蒸馏契约 / 细纲声明节拍钩子 / 细纲声明场景）、恩怨清算档位（原「反和解浓度」）落书仓契约而非文风铁律、废弃 AI 主动关键词检索，依据任务 07-07-knowledge-layer-rework 与 story-repo-spec 0.15；1.2 于 2026-07-04 修订 §4 #2 验收用词；1.1 于 2026-07-02 修订 §5 依赖口径一处；1.0 于 2026-06-12 作者逐项确认定稿）
> 日期：2026-06-12
...
│   ├── 题材流派指导.md          # ★ 建书蒸馏产物：本书写作契约（骨架约定/差异化点/专属毒点/节奏参数含恩怨清算档位）
│   └── 金句库/战斗.md …
├── 工作区/                     # ── 没定稿的一切，默认不进 git ──
...
- **爽点**：不是账本条目，是**知识层**——v6 的追读力分类（危机钩/悬念钩/渴望钩/情绪钩/选择钩）、爽点与节奏知识库、37 个题材模板**重写适配 v7 后继承**（题材与流派两维分立；条目只写结构与禁忌，不写示例情节）。消费走**声明路由**，无 AI 主动检索：建书时按题材/流派蒸馏成书仓契约（`文风/题材流派指导.md`）；细纲声明本章节拍/章尾钩子/本章场景，备料与两审按声明查表注入对应切片与毒点。
...
| 3 | 备料 | 脚本 | 组装 `工作区/本章写作材料.md`：全书近况+要写到的事+事实切片+信息差边界+近章结尾+反复读清单+文风锚点+题材流派契约（含恩怨清算档位）+细纲声明命中的知识切片；**默认用精准片段**（当前卷+上一卷时间线、按编号取账本条目） |
...
| 8 | AI 和解倾向毒害爽文 | 恩怨清算纪律分布在知识条目毒点（原「反和解」），随备料与两审按声明注入；档位（有仇必报/宿命必偿/恩怨分明/以直报怨/留有余地，spec 0.16 扩五档）按题材配默认、建书对谈作者可调、落书仓契约——约束主角行动线与恩怨结算，不禁配角人设 | 7.0 | 题材知识含恩怨清算默认档；样章对比可感 |
