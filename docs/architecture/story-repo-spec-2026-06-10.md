...
> 状态：0.17（2026-07-18 生效）。相对 0.16 的变更（决策 45-51 见 §14）：
...
`ID` 与名称用于 `本章对象` 精确引用；正名/别名等命名结果附着在对象上，不另建平行命名实体目录。prep 和事实审查只读取细纲显式声明的计划对象，并始终标明“计划不等于已发生事实”。

正文首次建立该对象时，事实审查可在定稿包给出 `factChanges`：每项含安全的 `planPath`、`factPath`、完整事实 Markdown、`decision`、可选 `expectedHash`/`resolution` 和 `removePlan`。`无冲突`可随章节原子转正；`冲突`/`歧义`整章零写入并要求作者裁决；`作者已裁决`必须记录裁决说明。事实文件、计划删除和章节正文属于同一 git commit 与同一精确回滚集合。
...
契约只通过显式 `persist-contract` 更新：作者确认的新版本必须严格递增，说明生效起章、原因和变更类型；默认只影响下一未定稿章及以后。作者主动修改、重大改纲或阻断性冲突立即呈报；同类非阻断问题连续两章或同卷三次在下一个作者停顿点呈报；卷末固定复盘。信号只产生修订候选，作者确认前不得落盘。更新会使生效章后的未定稿材料与审稿结果失效，已定稿章节仍走影响分析/吃书。
...
备料只读取命中选择的「落笔时」切片，编辑审只读取同一选择的「审稿时」切片；自定义声明和真实变体原样保留。手动定稿与暂存都在清理细纲前把最终选择写入章 front matter 的平铺 `知识选择` 列表（`维度｜名称`、`变体｜说明`），这是近期历史的唯一输入；未选候选不持久化，重复只降权和提醒，不硬禁有递进/回响/回收目的的复用。
...
| 2 | 确认细纲 | **作者** | 整份细纲的任务、章级知识选择、变体和计划对象引用一并确认（`自动确认细纲: true` 时默认采纳提案，跳过停顿） |
...
| 6 | 两审 | AI ×2（各自新鲜上下文） | **事实审查**核对设定/时间线/连贯/角色/逻辑、要写到的事、泄密候选、履历证据、计划对象与既有事实，并输出可选 `factChanges`；冲突/歧义须作者裁决。**编辑审**核对结构、节奏、商业性、主角动机、作品契约和所选章级知识的审稿切片；合理宽恕与有代价救赎合法，无成本洗白或无动机让渡底线才报问题。契约问题填写条款标识。两审输出结构化问题清单（severity + category + blocking）→ `工作区/评审报告/`。|
...
**定稿的一次 commit 包含**：草稿 → `定稿/正文/`（front matter 写入章档案、条目变动和细纲最终 `知识选择`）；`设定/` 变更（位置/状态/境界/持有/信息差/名册/时间线追加）；无冲突/已裁决的 `factChanges` 事实写入及对应计划删除；涉及的三类条目文件**创建与更新**；`摘要/章摘要/`（审稿单定稿版）；commit 成功后才清工作区。任一事实冲突、hash 漂移或写入故障都整章零提交并精确回滚本次文件集合。
...
51. **扩库只属于开发治理**：运行时不向作品作者提供知识提交、审核、评分回传或自动扩库入口。普通策展与架构变更分级执行，规则真源为 `v7/docs/knowledge/策展规则.md`；v7 尚未生产，0.17 不保留旧书迁移器、旧路径双读或旧 schema 兼容。
