# Story Repo 格式规格（v7 草案 0.16；0.17 目标已冻结）

> **0.17 目标状态（2026-07-16，尚未生效）**：知识库十维治理、作品契约、故事对象两阶段生命周期和章级候选契约已由任务 `07-10-knowledge-taxonomy-governance` 冻结，见 `v7/docs/knowledge/` 与 §14 决策 45-51。运行时框架子任务完成目录、schema、DTO、书仓、installer 与测试的一次性切换后，才将本规格升为现行 0.17；在此之前正文描述的 0.16 路径仍是代码实际格式，不建设双读兼容层。
>
> 状态：0.16。相对 0.15 的变更（2026-07-08 ima 蒸馏收编，决策 43-44 见 §14）：
> - §6.3 恩怨清算档位三档扩五档：有仇必报 | 宿命必偿 | 恩怨分明 | 以直报怨 | 留有余地（从狠到缓；语义随蒸馏范本注入，`knowledge-pack.js`）
> - 知识库收编 ima 蒸馏批次 20 条（题材/末世、流派 6 含诡秘流新立、节拍 PA-108~110、场景 5、追读钩子 5）+ 存量修正 9 件；路由表 38 条数据行（含表头共 39 个物理行），「诡秘」别名由克苏鲁移交诡秘流
>
...
规则入铁律走"三次同类否决→系统提议→作者确认"的流程，每条带出处。文风铁律只承载**作者品味**（禁词/禁句式/口癖/节奏偏好/来自否决的规则）；题材知识（含恩怨清算，原 0.14 的"反和解"节）归 §6.3 契约与知识条目毒点，不写进本文件。
...
### 6.3 题材流派指导 `文风/题材流派指导.md`

建书蒸馏产物：把知识库的题材/流派骨架与本书设定在建书对谈中对撞一次（"库怎么说 → 你怎么定"），固化成作者签字的**本书写作契约**。下游（备料、两审）只消费本文件，不回读运行时知识库——库升级不影响在写的书；作者手改本文件走手改补登（序 2 范围）。
...
流派:
  - 系统流
恩怨清算: 恩怨分明          # 有仇必报 | 宿命必偿 | 恩怨分明 | 以直报怨 | 留有余地（从狠到缓，0.16 扩五档）
来源:
  - 题材/玄幻.md@<hash>     # 或整行「对谈共创」（路由未命中时）
---
## 骨架约定
（本书遵循的题材/流派结构约定）
## 差异化点
（必填 ≥3 条：本书与流派常规的不同点——蒸馏是过滤器不是复印机）
## 本书专属毒点
（编辑审逐条核对）
## 节奏参数
（首打章数等；恩怨清算档位的本书化表述）
...
**恩怨清算**（原 0.14 §6.1"反和解"，AI 对齐训练的和解倾向对爽文是毒药，只能随写作材料前置反制）：纪律本体分布在知识条目毒点里，不设中央规则；档位约束**主角行动线与恩怨结算结果**，不约束配角人设——圣母/和事佬配角是合法设计与冲突来源，红线是其主张不得被叙事背书并软化结算（"留有余地"档除外）。毒点措辞一律"恩怨结算不得X/主角不得X"，禁止"不得出现X角色"。

**缺口降级**：知识库路由未命中 → 契约由对谈现场共创（`来源: 对谈共创`），流程不变；无本文件的书（存量/迁移书）备料跳过该锚点，全流程照常。
...
**知识声明位（0.15，声明即路由）**：`本章提案` 段内三条标签行——`本章节拍：`（知识库节拍编号或自定义文本）、`章尾钩子：`（追读钩子类型）、`本章场景：`（顿号分隔多值，场景名或自定义文本）。三者**皆可空**；起草细纲的 DTO 附节拍/场景紧凑索引与场景候选（脚本按场景关键词扫卷纲对应段与上一章结尾，只出候选不拦截），AI 从菜单点选或自定义。备料与两审按声明查知识库注入对应切片与毒点（§8 第 3/6 步）；**声明查不到表时注入声明文本本身**，不报错不挡流程。
...
| # | 阶段 | 执行体 | 文件效果 |
|---|------|--------|----------|
| 1 | 起草细纲 | 脚本读全书近况 + AI 拟提案 | 生成 `工作区/细纲.md`，含本章定位声明与知识声明位（节拍/钩子/场景，§7）；DTO 附知识紧凑索引与场景候选 |
| 2 | 确认细纲 | **作者** | 细纲的"要写到的事"固化（`自动确认细纲: true` 时默认采纳提案，跳过停顿） |
| 3 | 备料 | 脚本 | 组装 `工作区/本章写作材料.md`：全书近况+要写到的事+事实切片+信息差边界+近章结尾+反复读清单+文风锚点+题材流派契约（§6.3，无契约的书跳过）+细纲声明命中的知识切片（节拍/场景条目的「落笔时」节；查不到表注入声明文本本身）。**默认精准片段**（当前卷+上一卷时间线、按编号取条目、按需取正文段，§11.1） |
...
| 6 | 两审 | AI ×2（各自新鲜上下文） | **事实审查**（v6 的 5 维度：设定一致性、时间线、叙事连贯、角色一致性、逻辑 + v7 特有项：①"要写到的事"逐条核对正文；②机检泄密候选是否真泄密；③条目履历引用的章内证据是否属实；④**未登记伏笔检测**：搭整章通读反查"正文里出现疑似伏笔但无对应条目"，只出候选不拦截，见下）、**编辑审**（结构、节奏、商业性、主角动机：主角每个主要行动是否有即时驱动、可见压力、预期收益、情绪原因，或明确标注为非理性行为；**毒点核对**：审稿输入按 book.yaml 类型/流派与细纲声明附上契约专属毒点与命中知识条目毒点，逐条核对——恩怨清算类毒点只判叙事结果与主角行为，不判配角人设，§6.3）→ `工作区/评审报告/`。输出格式：结构化问题清单（severity + category + blocking），参考 v6 review schema。|
...
| 1 | 工作目录无任何书 / 当前书不存在 | **建书引导**：问答式生成 book.yaml（类型+流派按知识库路由表归一）、总纲、第一卷卷纲、题材流派契约（§6.3，蒸馏对撞；路由未命中→对谈共创），生成书仓库（含指路 AGENTS.md），登记进 `books.jsonl` |
...
| references（题材模板/追读力/爽点节奏） | 知识层重写适配后继承（0.15：题材/流派分维、声明路由，见 §5/§6.3/§7） |
...
47. **作品契约替代题材流派指导**：删除 `文风/题材流派指导.md`，新建 `作品契约/作品契约.md` 作为下游唯一书级真源，`知识选择记录.md` 保存候选、拒绝、自定义和来源证据。删除恩怨清算五档及题材默认值，改用“冲突与关系结算原则”核对动机、责任、代价、边界和读者承诺。
...
43. **恩怨清算档位三档扩五档**：有仇必报 | 宿命必偿 | 恩怨分明 | 以直报怨 | 留有余地（从狠到缓）。语义：有仇必报=结仇必清算、代价足额落地；宿命必偿=清算可迟到不缺席、因果闭环由叙事兜底兑现；恩怨分明=有恩报恩有仇报仇、等价对付不扩大化；以直报怨=以公正回应仇怨、威胁未除不滥慈悲、除害不虐杀；留有余地=结算保留回旋空间、允许有代价的宽宥。落点：`knowledge-pack.js` 蒸馏范本、`format.test.js` 档位校验、§6.3 契约示例。存量题材取值不回改；`宿命必偿` 暂无题材默认使用，`以直报怨` 由末世首个采用；建书对谈可在五档中选择。
...
40. **知识层从「平移继承」改「重写适配」，消费走声明路由**：v6「CSV + AI 主动关键词检索」废弃（与不变量"只吃整理好的 DTO"冲突，检索触发靠模型自觉不可测试）。知识注入全部由脚本按确定性键查表：书级键=book.yaml 类型+流派（建书蒸馏成 §6.3 契约）；章级键=细纲声明位 本章节拍/章尾钩子/本章场景（§7，备料与两审按声明注入切片与毒点）。题材与流派两维分立（题材=玄幻，流派=退婚流，可多选）；知识库是旁路增益不是必经关卡——路由未命中→对谈共创，声明查不到→声明本身即知识，无契约旧书照常。
41. **"反和解"更名"恩怨清算"并迁出文风铁律**：文风铁律回归纯作者品味；恩怨清算纪律分布在知识条目毒点（不设中央规则文件），档位（有仇必报/恩怨分明/留有余地）按题材配默认、建书对谈可调、落契约"节奏参数"节。档位约束主角行动线与恩怨结算结果，不禁配角人设（圣母/和事佬配角合法，红线=叙事背书）。
