...
> - 知识库收编 ima 蒸馏批次 20 条（题材/末世、流派 6 含诡秘流新立、节拍 PA-108~110、场景 5、追读钩子 5）+ 存量修正 9 件；路由表 38 行，「诡秘」别名由克苏鲁移交诡秘流
>
...
43. **恩怨清算档位三档扩五档**：有仇必报 | 宿命必偿 | 恩怨分明 | 以直报怨 | 留有余地（从狠到缓）。语义：有仇必报=结仇必清算、代价足额落地；宿命必偿=清算可迟到不缺席、因果闭环由叙事兜底兑现；恩怨分明=有恩报恩有仇报仇、等价对付不扩大化；以直报怨=以公正回应仇怨、威胁未除不滥慈悲、除害不虐杀；留有余地=结算保留回旋空间、允许有代价的宽宥。落点：`knowledge-pack.js` 蒸馏范本、`format.test.js` 档位校验、§6.3 契约示例。存量题材取值不回改；新档位暂无题材默认使用（末世=以直报怨 为首个），建书对谈可选。
