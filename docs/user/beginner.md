...
本教程带一次完整的真实界面走查：装运行环境 → 启动工作台 → 配置模型 → 打开工作区 → 建书 → 写第一章 → 定稿与恢复。命令与版本对应当前预览版，细节分支见各专题文档。
...
| **作品目录（工作区）** | 你的书仓所在目录。工作台只在你选定的这个目录内读写 | 准备一个空目录并备份 |
...
先准备一个专门存作品的**空目录**（可以带空格，路径中不要放密钥或作品备份），在该目录启动：
...
- 用别的服务（自建、代理、其他厂商）时点「稍后配置」，到 **设置 → 模型** 里添加。
...
2. 添加提供方后，把它设为当前主模型，然后回对话区发一条短消息试通。收到正常回答再开始建书。
...
点输入框上方的**选择工作区** → 「添加工作区…」，在目录浏览器里定位到第 2 步准备的目录：

![选择工作区目录](images/beginner/08-workspace-picker.png)

- 面包屑右侧的铅笔按钮可以直接键入路径。
- 左侧是当前位置的父级列表，右侧是当前目录内容；选中目录后点「打开」。
- 也可以用「新建文件夹」当场建一个作品目录。
...
- **定稿**：查看正文、审读结论与拟沉淀事实后再批准。成功应同时体现在定稿文件、账本／本书记忆的变化和版本提交上。
