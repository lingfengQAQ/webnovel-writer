...
本教程按装运行环境 → 启动工作台 → 配置模型 → 打开工作区 → 建书 → 写第一章 → 定稿与恢复组织。安装与截图基线为 **DSH 0.1.5-rc.2、Scriptor 0.1.0-preview.5、嵌入提供方 0.0.8**，细节分支见各专题文档。

截图取自独立演示环境（合成作品《灯塔来信》、占位凭据、未调用收费模型），只用于说明界面位置与状态文案。

**版本区别**：`v8` 分支包含尚未进入 preview.5 的快速设计与审核修复；安装已发布包不会获得这些未发行改动。本教程要求真实设计正文；preview.5 用户先按 [首书教程](first-book.md) 逐部设计，遇到模块缺失按 [排错](troubleshooting.md) 处理。最新发行内容以 [更新记录](../../CHANGELOG.md) 的具体版本为准。
...
dsh --profile scriptor --from-default-profile web --dump-config
dsh plugin --profile scriptor add @linfengqaqtat/dsh-scriptor-full@0.1.0-preview.5
```
...
本教程以图文为主，便于复制命令、检查字段和按步骤回看。现有图片覆盖启动、密钥引导、设置、目录选择、书房、编辑器和索引面板。**可视化、真实对话工具卡、作者确认弹窗和索引运行中状态尚未实拍**；对应文字按源码核对，不能当作真实模型端到端验收。采集时隐藏了本机路径，目录选择器用了应用内备用方案。

无需录制等待模型生成的完整长视频；操作容易错过的部分适合另外录制 30–90 秒短片。下面是拍摄分镜建议，**尚未交付视频**：

| 短片 | 分镜顺序 | 结束时核对 |
| --- | --- | --- |
| 主模型配置 | 设置 → 模型 → 添加自定义提供方 → 填协议/地址/模型 → 保存 → 选择模型 → 发短消息 | 正常回答；密钥输入与启动 token 全程避开画面 |
| 选中文本提问 | 打开合成稿 → 选中一段 → 引用到对话 → 提问 | 对话引用的书名、路径与所选内容一致 |
| 定稿确认 | 展示待审稿和审读结论 → 核对拟沉淀事实 → 作者批准 → 工具返回 → 查看定稿与历史 | 同时看文件与提交，不以批准动作结束录像 |
| 暂停与继续索引 | 展示配置和费用说明 → 手动更新 → 暂停 → 查看保留进度 → 继续 | 状态和进度真实变化；未配置服务时不能伪造已同步画面 |

拍摄只用合成作品；如果启用真实模型或检索服务，按服务方价格计费并单独记录。预置内容、模拟返回和真实调用必须在片头注明。
