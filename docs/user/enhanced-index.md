...
| 场景识别 | 宿主已配置的聊天模型 | 按段落长度分块 |
| 查询重排序 | 一个重排接口与模型 | 保持原排序 |
...
| 字段 | 怎么填 | 注意 |
| --- | --- | --- |
| **启用语义检索** | 勾选后才真正生效 | 只在调用混合检索时发送当前书的定稿片段与查询 |
...
模型或维度改过之后，**下次检索会重建旧向量**——这一点会消耗调用额度，改配置前先想清楚。
...
| **状态行** | 见下方状态表 |
| **提交后自动更新** | 勾选后，每次定稿提交都会触发后台更新；不勾选就只在点「立即更新」时跑 |
...
| **目标提交 / 已同步** | 目标提交是这本书最新一次版本提交的编号，已同步是索引已经覆盖到的那次提交；两者不同表示还有内容没入索引 |
...
「重建向量」是唯一会大范围重复消耗嵌入额度的操作；「重新识别场景」消耗的是聊天模型额度。

## 5. 数据流与费用边界

| 动作 | 发出去的内容 | 消耗 |
| --- | --- | --- |
| 启用语义检索后的检索 | 当前书的**定稿**片段与你的查询 | 嵌入服务 |
| 后台索引更新 | 定稿正文切片 | 嵌入服务 |
