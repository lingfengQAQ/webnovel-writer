    ...
    for ch in chapters:
        ...
        if "review_blocking" not in errors and args.mode in {"write", "nightly"}:
            commit_cmd = [
                ...
                "--chapter", str(ch),
                "--review-result", str(args.tmp_dir / "review_results.json"),
                "--fulfillment-result", str(args.tmp_dir / "fulfillment_result.json"),
                "--disambiguation-result", str(args.tmp_dir / "disambiguation_result.json"),
                "--extraction-result", str(args.tmp_dir / "extraction_result.json"),
            ]
...
def main() -> None:
    ...
    parser.add_argument("mode", choices=["write", "heal", "nightly"])
