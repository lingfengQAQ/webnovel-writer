...
每章必须包含：目标、阻力、代价、时间锚点、章内时间跨度、与上章时间差、倒计时状态、爽点、Strand、反派层级、视角/主角、关键实体、本章变化、章末未闭合问题、钩子、`章长方向`，以及结构化节点 `CBN`、`CPNs`、`CEN`、`必须覆盖节点`、`本章禁区`。
...
- **`章长方向` 记录本章走哪一档**（值域由项目自定，如 长/中/短）。
  它的排布来自 Step 4 的卷节拍表——**Step 7 生成章纲时必须把每章的排布落进这个字段**，
  不要只留在节拍表里：写手打开的是章纲，判据不在章纲里等于没有。
  **本字段只写档位方向，不写字数区间**——区间属于项目自己的文风判据，
  应留在项目的判据文件中，章纲里写指针即可，避免同一组数字散在多处、改一处漏一处。
...
- 每章都有 `章长方向`。
- 该字段与 Step 4 节拍表里的排布一致；不一致时报出章号，不自动改写。
