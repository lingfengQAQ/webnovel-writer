...
每章必须包含：目标、阻力、代价、时间锚点、章内时间跨度、与上章时间差、倒计时状态、爽点、Strand、反派层级、视角/主角、关键实体、本章变化、章末未闭合问题、钩子、`接缝`，以及结构化节点 `CBN`、`CPNs`、`CEN`、`必须覆盖节点`、`本章禁区`。
...
- **`接缝` 字段记录正文层的衔接点**，两行，相邻两章必须互指同一拍：
  - A 章：`本章正文实际停在哪一拍`
  - B 章：`从上一章的哪一拍之后接`

  写法要求：写成正文里那一拍的动作，不要写成 `CEN` / `钩子` / 节点编号。
  **`CEN`（或钩子）不等于正文的最后一拍**：只要 A 章在 `CEN` 之后还有收尾动作
  （落主题句、收束画面、章末留一个动作），A 章正文的最后一拍就不是 `CEN`。
  B 章若按「从钩子之后接着写」去写，会把那几拍重写一遍。
  接缝也可以是有意跳空（A 章停在某处、B 章从另一处起，中间不交代）——
  此时 B 章的 `接缝` 要显式写明「中间不交代」，避免写手补一段过渡把留白填掉。
...
`接缝` 一致性检查（逐对相邻章，有该字段时执行）：

- A 章的 `本章正文实际停在哪一拍` 与 B 章的 `从上一章的哪一拍之后接` 指的是同一拍。
- 若 A 章的「实际停在哪一拍」不是 `CEN` 本身，B 章的 `接缝` 里不得出现
  「从钩子之后接着写」「从 `CEN` 之后接着写」这类表述 —— 那会漏掉 `CEN` 之后的收尾拍。
- 两侧都不得只写节点编号（`CEN` / `CBN` / 钩子）代替具体那一拍。
