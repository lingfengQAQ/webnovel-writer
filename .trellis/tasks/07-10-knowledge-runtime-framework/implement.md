...
- [x] 1.5 删除全量章级索引的运行时出口，补 canonical 不等于创意选择、缺目录降级和候选限量测试。
...
- [x] 4.1 draft-outline DTO 改为按本章语料查询四维少量候选，不再发送全量菜单。
- [x] 4.2 一次性切换细纲声明到节拍/场景/技法/追读/变体/对象，并允许多选与自定义。
- [x] 4.3 prepare 只注入所选落笔切片，review 只注入所选审稿切片；规划切片确有消费者。
- [x] 4.4 手动 finalize 与 stage-chapter 在清理细纲前固化 `知识选择`，finalize-batch 保持同构。
- [x] 4.5 从近期定稿章和批内前章读取历史，只做降权与重复提醒，合法递进不硬禁。
- [x] 4.6 补候选限量、整体确认、自定义降级、精准切片、近期软信号和手动/批量一致性测试后提交“章级调用”单元。
