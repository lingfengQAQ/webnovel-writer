...
> 版本：基线 1.1（2026-07-29 增补 §6 输入→路径白名单）。依据：PRD §1.4 产品原则 3/4、§3.5 状态机、§5 非功能需求（健壮性）；story-repo-spec 0.6 不变量 4、§10；story-repo-spec 0.19 决策 62。
...
## 6. 输入→路径白名单（2026-07-29 增补，基线 1.1）

6.1 **任何外部输入（AI 回流的 payload/JSON、命令行参数）进入文件路径之前，必须过白名单校验，校验拒绝而非静默改写**：

- 文件名干类（信息差 id、伏笔条目 id 等）：`isSafeFileStem`（`util/filename.js`，与 `sanitizeFileName` 单源，「净化后与原值相同」为判定基准）。
- 仓库相对路径类：`normalizePosixRelative`（`knowledge/design.js`，拒绝 `\`、绝对路径、`../`、`/../`）。
- 章号/卷号类：`parsePositiveInt`（`util/positive-int.js`，十进制正整数；替换 `parseInt + isNaN`，后者拦不住 `"3.5"` 截断、`"1e3"`、负数、0）。

6.2 **两层防线**：源头层（finalize/staging 共用 `knowledge/payload-guards.js`，R6 两链路同一函数不开第二条口径）人话报错带索引定位（`secretWrites[i].id`），AI 可改 payload 重试；写入器层不信任任何调用方、内建同款校验。

6.3 **`writeAtomicBatch` 总闸持有词法级 repo 边界断言**：任一目标路径解析后不在 repoPath 内即整批拒绝；判定在 `mkdir` 之前（越界零建目录），回滚语义不变。总闸是不信任调用方的机械断言，防未来新调用点再漏接。

6.4 非法输入按「需作者确认」级处理：人话报错、整批拒绝、零写入零提交，禁止静默净化或部分落盘。
